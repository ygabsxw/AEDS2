...
void partialInsertionSort(Pokemon findPokemon[], int tam, int k, int *comp, int *mov) {
    for (int i = 1 ; i < tam ; i++) {
            Pokemon key = findPokemon[i];
            int j = i - 1;

            while (j >= 0) {
                int date = compareDates(&key.captureDate, &findPokemon[j].captureDate);
...
int main () {
    ...
    partialInsertionSort(findPokemon, j, k, &comp, &mov);
    ...
    FILE *arquivo = fopen("843610_partialInsertionSort.txt", "w");
    if (arquivo == NULL) {
