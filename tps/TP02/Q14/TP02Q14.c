...
void countSort(Pokemon findPokemon[], int tam, int exp, int *comp, int *mov) {
    ...
    // Modifica count[] para armazenar as posições corretas dos Pokémons na saída
    for (int i = 1; i < 10; i++) {
        count[i] += count[i - 1];
    }
    ...
    // Copia o array de saída para o array original findPokemon[]
    for (int i = 0; i < tam; i++) {
...
int main () {
    ...
    radixSort(findPokemon, j, &comp, &mov);
    ...
    FILE *arquivo = fopen("843610_radixsort.txt", "w");
    if (arquivo == NULL) {
