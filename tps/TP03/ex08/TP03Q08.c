...
int main () {
    ...
    ListaDupla listaPokemons; 
    inicializarListaDupla(&listaPokemons);
    ...
    quickSort(listaPokemons.inicio, listaPokemons.fim, comp, mov);

    clock_t end = clock();

    imprimirListaDupla(&listaPokemons);

    double executionTime = ((double)(end - start)) / CLOCKS_PER_SEC * 1000.0;

    // txt

    FILE *arquivo = fopen("843610_quicksort2.txt", "w");
    if (arquivo == NULL) {
